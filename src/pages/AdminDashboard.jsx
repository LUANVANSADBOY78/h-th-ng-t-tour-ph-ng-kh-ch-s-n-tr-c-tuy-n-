...
export default function AdminDashboard({ onExit }) {
    ...
    const filtered = useMemo(() => bookings.filter((booking) => `${booking.code} ${booking.customer} ${booking.service}`.toLowerCase().includes(query.toLowerCase())), [bookings, query]);

    const confirmBooking = (code) => {
        setBookings((current) => current.map((booking) => booking.code === code ? {...booking, status: 'Da xac nhan' } : booking));
        setNotice(`Da xac nhan don ${code}.`);
    };
    ...
    const rows = filtered.map((booking) => e('tr', { key: booking.code },
        ...
        e('td', null, e('span', { className: `admin-status ${STATUS_CLASS[booking.status]}` }, booking.status)),
        e('td', null, booking.status === 'Cho thanh toan' ?
            e('button', { className: 'confirm-button', onClick: () => confirmBooking(booking.code) }, 'Xac nhan') :
            e('button', { className: 'view-button', onClick: () => setNotice(`Dang xem chi tiet don ${booking.code}.`) }, 'Chi tiet'))
    ));
        ...
        e('section', { className: 'admin-main' },
            ...
            e('section', { className: 'admin-panel' },
                e('div', { className: 'admin-panel-heading' },
                    ...
                    showBookings && e('input', { 'aria-label': 'Tim don dat cho', value: query, onChange: (event) => setQuery(event.target.value), placeholder: 'Tim ma don, khach hang...' })
                ),
